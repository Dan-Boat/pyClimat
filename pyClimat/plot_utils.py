...
# customized plots
GryBr = matplotlib.colors.LinearSegmentedColormap.from_list(
    "my_gradient",
    (
...
GryBr_r = matplotlib.colors.LinearSegmentedColormap.from_list(
    "my_gradient",
    (
...
PrecAno = matplotlib.colors.LinearSegmentedColormap.from_list(
...
corr = matplotlib.colors.LinearSegmentedColormap.from_list(
    "correlation",
    (
...
vegetation = matplotlib.colors.LinearSegmentedColormap.from_list(
    "correlation",
    (
...
causal = matplotlib.colors.LinearSegmentedColormap.from_list(
    "my_gradient",
    (
...
PrecipitationCal = matplotlib.colors.LinearSegmentedColormap.from_list(
    "my_gradient",
    (
...
def plot_background(
    ...
    if plot_coastlines == True:
        if coast_resolution is None:
            p.axes.coastlines(
                resolution="50m", linewidth=1.5, color=coast_color
            )  # add coastlines outlines to the current axis (110m, 50m , 10m)

        else:
            p.axes.coastlines(
                resolution=coast_resolution, linewidth=1.5, color=coast_color
            )  # add coastlines outlines to the current axis

    if plot_borders == True:
        p.axes.add_feature(
            cfeature.BORDERS, edgecolor="grey", linewidth=0.5
        )  # adding country boarder lines
    ...
    if domain is not None:
        if domain == "Europe":  # Europe
            ...
            domain == "Tibetan Plateau" or domain == "Himalayas"
        ):  # Tibet Plateau/Himalayas
            minLon = 40
            maxLon = 120
            minLat = 0
            maxLat = 60
...
class FixedPointNormalized(matplotlib.colors.Normalize):
    def __init__(self, vmin=None, vmax=None, sealevel=0, color_val=0.21875, clip=False):
