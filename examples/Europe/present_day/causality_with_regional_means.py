...
def perform_causality_testing_with_regional_means(maxlon, minlon, maxlat, minlat, Y_varname=None,
    ...
    if season == "DJF":
        ...
        ea_index_echam = xr.DataArray(df_echam_pcs[str(2)] * -1, dims="time", coords={"time": df_echam_pcs["time"]})
...
def plot_causal_to_climate(season="DJF"):
    ...
    axes = [ax1, ax2, ax3]
    titles = ["(a) Y(NAO, EA) to X(Prec) " + season, "(b)  Y(NAO, EA) to X(t2m) "+ season,  
              "(c)  Y(NAO, EA) to X($\delta^{18}$Op) "+ season]
    ...
    plt.savefig(os.path.join(path_to_plots, season +"_causal_regional_means_nao_ea_to_climate.svg"), format= "svg", 
                bbox_inches="tight", dpi=300)


if __name__ == "__main__":
    plot_causal_EA_to_nao(season="JJA")
    plot_causal_EA_to_nao(season="DJF")
    
    plot_causal_to_climate(season="JJA")
    plot_causal_to_climate(season="DJF")