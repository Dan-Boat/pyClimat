...
def cal_diff_nao_ea_index(varname, index_same, index_op,
                          lev=None, lev_units=None, season="DJF", units=None):
...
def perform_composite_DJF():
    ...
    d18op_diff = cal_diff_nao_ea_index(varname ="d18op", units="per mil", index_same=nao_same_echam_positive, index_op=nao_op_echam,
                              lev=None, lev_units=None, season="DJF")   
    
    temp_diff = cal_diff_nao_ea_index(varname ="temp2", units="°C", index_same=nao_same_echam_positive, index_op=nao_op_echam,
                              lev=None, lev_units=None, season="DJF")
    
    prec_diff = cal_diff_nao_ea_index(varname ="prec", units="mm/month", index_same=nao_same_echam_positive, index_op=nao_op_echam,
                              lev=None, lev_units=None, season="DJF")
    ...
    plt.savefig(os.path.join(path_to_plots, "composite_nao_ea_djf.svg"), format= "svg", bbox_inches="tight", dpi=300)
    plt.show()   
    
    
def perform_composite_JJA():
        
    # read all the required datsets (for winter)
    df_era_pcs = pd.read_csv(PCs_ERA_JJA_path , parse_dates=["time"])
    df_echam_pcs = pd.read_csv(PCs_ECHAM_JJA_path, parse_dates=["time"])
    
    # read only the times related to the index sign
    same = (df_echam_pcs["1"] >0)&(df_echam_pcs["2"] *-1 > 0) | (df_echam_pcs["1"] <0)&(df_echam_pcs["2"] *-1 < 0)
    same_positive = (df_echam_pcs["1"] >0)&(df_echam_pcs["2"] *-1 > 0) 
    op = ((df_echam_pcs["1"] >0)&(df_echam_pcs["2"] *-1 < 0) | (df_echam_pcs["1"] <0)&(df_echam_pcs["2"] * -1 > 0))
    ...
    nao_same_echam = xr.DataArray(df_echam_nao_ea_same[str(1)], dims="time", coords={"time": df_echam_nao_ea_same["time"]})
    nao_same_echam_positive = xr.DataArray(df_echam_nao_ea_same_positive[str(1)], dims="time", coords={"time": df_echam_nao_ea_same_positive["time"]})
    
    nao_op_echam = xr.DataArray(df_echam_nao_ea_op[str(1)], dims="time", coords={"time": df_echam_nao_ea_op["time"]})
    ...
    d18op_nao_same_sval, d18op_nao_same_pval, d18op_nao_same_sig = perform_correlation(atmos_index=nao_same_echam, 
                                                                        varname="d18op", units="per mil",
                                                                        filename="d18op_nao_same_scc.nc", season="JJA")
    
    d18op_nao_op_sval, d18op_nao_op_pval, d18op_nao_op_sig = perform_correlation(atmos_index=nao_op_echam, 
                                                                        varname="d18op", units="per mil",
                                                                        filename="d18op_nao_op_scc.nc", season="JJA")
    ...
    d18op_diff = cal_diff_nao_ea_index(varname ="d18op", units="per mil", index_same=nao_same_echam_positive, index_op=nao_op_echam,
                              lev=None, lev_units=None, season="JJA")   
    ...
    plt.tight_layout()
    plt.subplots_adjust(left=0.05, right=0.88, top=0.94, bottom=0.10, hspace=0.001)
    plt.savefig(os.path.join(path_to_plots, "composite_nao_ea_jja.svg"), format= "svg", bbox_inches="tight", dpi=300)
    plt.show()

if __name__ == "__main__":
    perform_composite_DJF()
    perform_composite_JJA()