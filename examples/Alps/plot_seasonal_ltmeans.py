...
exp_name_0 = "a003_hpc-bw_e5w2.3_t159_PI_Alps_east_0_t159l31.6h"
exp_name_150 = "a001_hpc-bw_e5w2.3_t159_PI_Alps_east_300_t159l31.6h"
years= "1003_1017"
period = "1m"
...
# visualising long-term seasonal means
...
plot_seasonal_mean(variable='$\delta^{18}$Op vs SMOW', data_slt=Alps_150_d18op_slt , cmap=RdBu, units="‰", seasons=["JJA", "DJF"], 
                   ...
                   season_label= ["[A]       Alps east 150% [JJA]", "[B]        Alps east 150% [DJF]"])
...
plt.savefig(os.path.join(path_to_store, "figS5.1.png"), format= "png", bbox_inches="tight", dpi=600)
...
plot_seasonal_mean(variable='$\delta^{18}$Op vs SMOW', data_slt=Alps_0_d18op_slt , cmap=RdBu, units="‰", seasons=["JJA", "DJF"], 
                    axes=[ax1,ax2], fig=fig, vmax=5, vmin=-20, levels=22, domain="Europe", level_ticks=11, cbar_pos = [0.90, 0.69, 0.02, 0.25], title=True,
                    season_label= ["[A]       Alps east 0% [JJA]", "[B]        Alps east 0% [DJF]"])
...
plt.tight_layout() 
plt.subplots_adjust(left=0.05, right=0.89, top=0.94, bottom=0.06)
plt.savefig(os.path.join(path_to_store, "figS4.1.png"), format= "png", bbox_inches="tight", dpi=600)
