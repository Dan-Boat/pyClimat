...
path_to_data = (
    "/Volumes/Daniel_Book/Datasets/topo/Miotopofiles/CTL_Mio_Herold/jan_surf_files"
)
path_to_plots = (
    "/Users/danielboateng/Desktop/git_source_files/pyClimat/examples/Alps/Miocene/plots"
)

mmco_slm_path = "/Volumes/Daniel_Book/Datasets/topo/Miotopofiles/CTL_Mio_Herold/topo_Herold_Miocene_2160x1080_SLM.nc"
...
def extract_topo_profile(maxlon=20, minlon=-5, maxlat=47, minlat=46, dim="lon"):
    w1e1_mio = extract_profile(
        data=W1E1_topo,
        maxlon=maxlon,
        minlon=minlon,
        maxlat=maxlat,
...
def plot_topo_profiles(
    ...
    data_topo.plot(
        ax=ax, linestyle="-", color=colors, linewidth=3
    ...
    if title is not None:
        ax.set_title(
            title, fontdict={"fontsize": 22, "fontweight": "bold"}, loc="center"
        )
    ...
    if path_to_store is not None:
        plt.savefig(
            os.path.join(path_to_store, filename), bbox_inches="tight", format="svg"
        )
...
# reading data
CTL_topo = read_jan_surf_oromea(
    path=path_to_data, filename=CTL_filename, return_slm=False
)
W1E1_topo = read_jan_surf_oromea(
    path=path_to_data, filename=W1E1_filename, return_slm=False
)
W2E1_topo = read_jan_surf_oromea(
    path=path_to_data,
    filename=W2E1_filename,
)
W2E15_topo = read_jan_surf_oromea(
    path=path_to_data,
    filename=W2E15_filename,
)
W2E0_topo = read_jan_surf_oromea(
    path=path_to_data,
    filename=W2E0_filename,
)
W2E2_topo = read_jan_surf_oromea(
    path=path_to_data,
    filename=W2E2_filename,
)
...
def plot_global_topo():
    ...
    fig, ax = plt.subplots(
        nrows=1, ncols=1, figsize=(13, 12), subplot_kw={"projection": projection}
    )
    ...
    plot_echam_topo(
        ...
        level_ticks=6,
        cbar=True,
        cbar_position=[0.35, 0.05, 0.45, 0.02],
    ...
    plt.savefig(
        os.path.join(path_to_plots, "global_topo.svg"),
        format="svg",
        bbox_inches="tight",
        dpi=600,
    )
...
def plot_topo_Alps():
    ...
    plt.subplots_adjust(left=0.05, right=0.89, top=0.95)
    plt.savefig(
        os.path.join(path_to_plots, "herold_topography.pdf"), format="pdf", dpi=600
    )
...
def plot_topo_exps():
    ...
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(
        nrows=2, ncols=2, figsize=(24, 22), subplot_kw={"projection": projection}
    )
    ...
    for i, label in enumerate(labels):
        if i == 0:
            plot_echam_topo(
                ...
                level_ticks=6,
                cbar=True,
                cbar_position=[0.35, 0.05, 0.25, 0.02],
                cbar_orientation="horizontal",
                norm=norm,
                plot_coastlines=False,
                bottom_labels=True,
    ...
    plt.savefig(
        os.path.join(path_to_plots, "topo_exps.svg"),
        format="svg",
        bbox_inches="tight",
        dpi=600,
    )
...
def plot_topo_exp_profiles():
    df_lon = extract_topo_profile(maxlon=20, minlon=-5, maxlat=47, minlat=46, dim="lon")

    df_lat = extract_topo_profile(maxlon=10, minlon=9, maxlat=54, minlat=40, dim="lat")
    ...
    plt.subplots_adjust(left=0.05, right=0.95, top=0.97, bottom=0.05)
    plt.savefig(
        os.path.join(path_to_plots, "topo_profiles.svg"),
        format="svg",
        bbox_inches="tight",
        dpi=600,
    )
...
def plot_topo_with_profile_lines():
    ...
    plot_echam_topo(
        ...
        level_ticks=6,
        cbar=True,
        cbar_position=[0.30, 0.05, 0.45, 0.02],
        cbar_orientation="horizontal",
        norm=norm,
        plot_coastlines=False,
        bottom_labels=False,
    ...
    plt.subplots_adjust(left=0.05, right=0.89, top=0.95)
    plt.savefig(
        os.path.join(path_to_plots, "W1E1_topo_with_lines.svg"),
        format="svg",
        bbox_inches="tight",
        dpi=600,
    )
...
def plot_topo_with_profile_transect():
    ...
    plt.subplots_adjust(left=0.05, right=0.89, top=0.95)
    plt.savefig(
        os.path.join(path_to_plots, "W1E1_topo_with_transects.svg"),
        format="svg",
        bbox_inches="tight",
        dpi=600,
    )
