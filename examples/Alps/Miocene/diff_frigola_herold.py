# -*- coding: utf-8 -*-
"""
Created on Wed Oct 25 17:20:52 2023
...
W1E1_278_fri_filename = "a011_hpc-bw_e5w2.3_t159_MIO_278ppm_CTL_t159l31.6h"
W1E1_450_fri_filename = "a013_hpc-bw_e5w2.3_t159_MIO_450ppm_CTL_t159l31.6h"
...
W1E1_278_fri_data, W1E1_278_fri_wiso = read_ECHAM_processed(main_path=path_to_data, exp_name=W1E1_278_fri_filename, years=years, 
                                          period=period, read_wiso=True)

W1E1_450_fri_data, W1E1_450_fri_wiso = read_ECHAM_processed(main_path=path_to_data, exp_name=W1E1_450_fri_filename, years=years, 
                                          period=period, read_wiso=True)


# extract values and compute long-term means

def extract_vars_and_analysis(data, wiso, fri_data, fri_wiso):
    ...
    temp2_pi = extract_var(Dataset=fri_data , varname="temp2", units="°C")
    prec_pi = extract_var(Dataset= fri_data , varname="prec", units="mm/month")
    d18op_pi = extract_var(Dataset=fri_data , varname="d18op", units="per mil", Dataset_wiso= fri_wiso)
...
# read data 
Mio278_data = extract_vars_and_analysis(data=W1E1_278_data, wiso=W1E1_278_wiso, fri_data=W1E1_278_fri_data, 
                                        fri_wiso=W1E1_278_fri_wiso)
Mio450_data = extract_vars_and_analysis(data=W1E1_450_data, wiso=W1E1_450_wiso, fri_data=W1E1_450_fri_data, 
                                        fri_wiso=W1E1_450_fri_wiso)
...
plot_annual_mean(variable="$\delta^{18}$Op vs SMOW difference", data_alt=Mio278_data.get("d18Op"), ax=ax1,
                ...
                plot_projection=projection, title="(a) MIO 278ppm ( H08-F18)", orientation="horizontal",
                cbar_pos= [0.05, 0.05, 0.25, 0.02])

plot_annual_mean(variable="Precipitation difference", data_alt=Mio278_data.get("precipitation"), ax=ax2,
                ...
                left_labels=False, fig=fig, plot_borders=False, sea_land_mask=mio_slm,
                plot_projection=projection, title="(b) MIO 278ppm ( H08-F18)", orientation="horizontal",
                cbar_pos= [0.35, 0.05, 0.25, 0.02])

plot_annual_mean(variable="Temperature difference", data_alt=Mio278_data.get("temperature"), ax=ax3,
                 cmap="RdBu_r", units="°C", vmax=10, vmin=-10, 
                levels=22, level_ticks=11, add_colorbar=True, plot_coastlines=False, bottom_labels=False,
                left_labels=False, fig=fig, plot_borders=False, sea_land_mask=mio_slm,
                plot_projection=projection, title="(c) MIO 278ppm ( H08-F18)", orientation="horizontal",
                cbar_pos= [0.65, 0.05, 0.25, 0.02])


plot_annual_mean(variable="$\delta^{18}$Op vs SMOW difference", data_alt=Mio450_data.get("d18Op"), ax=ax4,
                 cmap=GryBr_r, units="‰", vmax=5, vmin=-5, 
                levels=22, level_ticks=11, add_colorbar=False, plot_coastlines=False, bottom_labels=False,
                left_labels=False, fig=fig, plot_borders=False, sea_land_mask=mio_slm,
                plot_projection=projection, title="(e) MIO 450ppm ( H08-F18)", center=False,)

plot_annual_mean(variable="Precipitation difference", data_alt=Mio450_data.get("precipitation"), ax=ax5,
                 cmap=PrecAno, units="mm/month", vmax=150, vmin=-150, 
                levels=22, level_ticks=7, add_colorbar=False, plot_coastlines=False, bottom_labels=False,
                left_labels=False, fig=fig, plot_borders=False, sea_land_mask=mio_slm,
                plot_projection=projection, title="(f) MIO 450ppm ( H08-F18)",)

plot_annual_mean(variable="Temperature difference", data_alt=Mio450_data.get("temperature"), ax=ax6,
                 cmap="RdBu_r", units="°C", vmax=10, vmin=-10, 
                levels=22, level_ticks=11, add_colorbar=False, plot_coastlines=False, bottom_labels=False,
                left_labels=False, fig=fig, plot_borders=False, sea_land_mask=mio_slm,
                plot_projection=projection, title="(g) MIO 450ppm ( H08-F18)",)


fig.canvas.draw()   # the only way to apply tight_layout to matplotlib and cartopy is to apply canvas firt 
plt.tight_layout() 
plt.subplots_adjust(left=0.05, right=0.89, top=0.95, bottom=0.10, wspace=0.05,)
plt.savefig(os.path.join(path_to_plots, "global_diff.svg"), format= "svg", bbox_inches="tight", dpi=600)
plt.show()