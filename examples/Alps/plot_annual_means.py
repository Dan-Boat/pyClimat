...
This is example script for using Climat in visualising ECHAM module output (long-term annual means)
...
#west 
lat_w, h_w = 44 , 2.5  # lat and height
...
plot_annual_mean(ax=ax1, variable="Elevation", data_alt=elev_alt, cmap=Greys, units="m", vmax=3000, vmin=0, domain="Europe", 
                 levels=22, level_ticks=6, title="[A]")
# adding transects 
...
# d18Op
plot_annual_mean(ax=ax2, variable='$\delta^{18}$Op vs SMOW', data_alt=d18op_alt, cmap=RdBu, units="‰", vmax=2, vmin=-16, domain="Europe", 
                 levels=22, level_ticks=10, GNIP_data=df_gnip , title="[B]",)
...
plt.savefig(os.path.join(path_to_store, "fig1_new.svg"), format= "svg", bbox_inches="tight", dpi=600)